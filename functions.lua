...
local muted_players = {}
local check_area_requests = minetest.settings:get_bool("tpr.enable_area_requests")

local map_size = 30912
function tpr.can_teleport(to)
...
function tpr.tpr_teleport_player()
	target_coords = source:get_pos()
	local target_sound = target:get_pos()
	target:set_pos(tpr.find_free_position_near(target_coords))
	...
	--tpr.parti2(target_coords)
end

-- TPC & TPJ
function tpr.tpc_teleport_player(player)
	local pname = minetest.get_player_by_name(player)
	minetest.sound_play("tpr_warp", {pos = pname:get_pos(), gain = 0.5, max_hear_distance = 10})
	pname:set_pos(tpr.find_free_position_near(target_coords))
	minetest.sound_play("tpr_warp", {pos = target_coords, gain = 0.5, max_hear_distance = 10})
	--tpr.parti2(target_coords)
end

-- TPP
function tpr.tpp_teleport_player(player, pos)
	local pname = minetest.get_player_by_name(player)
	minetest.sound_play("tpr_warp", {pos = pname:get_pos(), gain = 0.5, max_hear_distance = 10})
	pname:set_pos(tpr.find_free_position_near(pos))
	minetest.sound_play("tpr_warp", {pos = pos, gain = 0.5, max_hear_distance = 10})
	--tpr.parti2(target_coords)
end

function tpr.find_free_position_near(pos)
	local tries = {
		{x=1,y=0,z=0},
		{x=-1,y=0,z=0},
		{x=0,y=0,z=1},
...
function tpr.tpr_mute(player, muted_player)
...
function tpr.tpr_unmute(player, muted_player)
	if muted_player == "" then
...
function tpr.tpr_send(sender, receiver)
	...
	if spam_prevention[receiver] == sender and not minetest.check_player_privs(sender, {tp_admin = true}) then
		minetest.chat_send_player(sender, S("Wait @1 seconds before you can send teleport requests to @2 again.", tpr.timeout_delay, receiver))

		minetest.after(tpr.timeout_delay, function(name)
	...
	-- Compatibility with beerchat
		...
		if minetest.check_player_privs(sender, {tp_admin = true}) and tpr.enable_immediate_teleport then
			if receiver == "" then
				minetest.chat_send_player(sender, S("Usage: /tpr <Player name>"))
					if minetest.get_modpath("chat2") then
						chat2.send_message(minetest.get_player_by_name(sender), S("Usage: /tpr <Player name>"), 0xFFFFFF)
			...
			tpr.tpr_list[receiver] = sender
			tpr.tpr_accept(receiver)
			...
			if minetest.get_modpath("chat2") then
				...
				chat2.send_message(minetest.get_player_by_name(sender), S("Teleport request sent! It will timeout in @1 seconds.", tpr.timeout_delay), 0xFFFFFF)
			end

			minetest.chat_send_player(receiver, S("@1 is requesting to teleport to you. /tpy to accept.", sender))
			minetest.chat_send_player(sender, S("Teleport request sent! It will timeout in @1 seconds.", tpr.timeout_delay))

			-- Write name values to list and clear old values.
			tpr.tpr_list[receiver] = sender
			tpr.tpn_list[sender] = receiver

			-- Teleport timeout delay
			minetest.after(tpr.timeout_delay, function(name)
				if tpr.tpr_list[name] and tpr.tpn_list[sender] then
					tpr.tpr_list[name] = nil
					tpr.tpn_list[sender] = nil
...
function tpr.tphr_send(sender, receiver)
	-- Check if the sender is muted
	if muted_players[receiver] == sender and not minetest.check_player_privs(sender, {server = true}) then
		minetest.chat_send_player(sender, S("Cannot send request to @1 (you have been muted).", receiver))
		return
	...
	-- Compatibility with beerchat
		...
		if minetest.check_player_privs(sender, {tp_admin = true}) and tpr.enable_immediate_teleport then
			...
			-- Teleport timeout delay
			minetest.after(tpr.timeout_delay, function(name)
				if tpr.tphr_list[name] and tpr.tpn_list[sender] then
...
function tpr.tpc_send(sender, coordinates)
	...
	if tpr.can_teleport(target_coords) == false then
	...
	-- Admin user (privilege `tp_admin`) overrides all protection
	if minetest.check_player_privs(pname, {tp_admin = true}) then
		tpr.tpc_teleport_player(sender)
		target_coords = nil
		...
		if minetest.check_player_privs(pname, {tp_tpc = true}) then
			...
			if protected then
				-- Now, we can safely check if the `check_area_requests` option is enabled (see the `config.lua` file, lines 28-35)
				if minetest.get_modpath("areas") and check_area_requests then
					for _, area in pairs(areas:getAreasAtPos(target_coords)) do
						if minetest.get_player_by_name(area.owner) then -- Check if area owners are online
							-- Spam prevention
							if spam_prevention[area.owner] == sender and not minetest.check_player_privs(sender, {tp_admin = true}) then
								minetest.chat_send_player(sender, S("Wait @1 seconds before you can send area requests to @2 again.", tpr.timeout_delay, area.owner))

								minetest.after(tpr.timeout_delay, function(name)
									spam_prevention[name] = nil
									if band == true then return end

									if spam_prevention[area.owner] == nil then
										minetest.chat_send_player(sender, S("You can now send area requests to @1.", area.owner))
										band = true
									end
								end, area.owner)
								return -- Otherwise, the code below will run, plus this code, too,
									   -- and we only want this code to run if the condition above is true.
							end
							...
							" It will timeout in @2 seconds.", table.concat(areas:getNodeOwners(tpc_target_coords[area.owner]), S(", or ")), tpr.timeout_delay))
							...
							if minetest.get_modpath("chat2") then
								...
								" It will timeout in @2 seconds.", table.concat(areas:getNodeOwners(tpc_target_coords[area.owner]), S(", or ")), tpr.timeout_delay), 0xFFFFFF)
							...
							tpr.tpc_list[area.owner] = sender
							tpr.tpn_list[sender] = area.owner

							minetest.after(tpr.timeout_delay, function(name)
								if tpr.tpc_list[name] and tpr.tpn_list[sender] then
...
function tpr.tpr_deny(name)
	local no_areas_message = "Usage: /tpn allows you to deny teleport requests sent to you by other players."
	local areas_message = "Usage: /tpn allows you to deny teleport/area requests sent to you by other players."

	if not tpr.tpr_list[name] and not tpr.tphr_list[name]
	and not tpr.tpc_list[name] and not tpr.tpn_list[name] then
		if check_area_requests then
			minetest.chat_send_player(name, S(areas_message))
			if minetest.get_modpath("chat2") then
				chat2.send_message(minetest.get_player_by_name(name), S(areas_message), 0xFFFFFF)
			end
		else
			minetest.chat_send_player(name, S(no_areas_message))
			if minetest.get_modpath("chat2") then
				chat2.send_message(minetest.get_player_by_name(name), S(no_areas_message), 0xFFFFFF)
			end
		end
	end

	-- Area requests
	if check_area_requests and tpr.tpc_list[name] then
		name2 = tpr.tpc_list[name]
	...
	elseif tpr.tphr_list[name] then
		name2 = tpr.tphr_list[name]
		minetest.chat_send_player(name2, S("Teleport request denied."))
		minetest.chat_send_player(name, S("You denied the request @1 sent you.", name2))
		if minetest.get_modpath("chat2") then
			chat2.send_message(minetest.get_player_by_name(name2), S("Teleport request denied."), 0xFFFFFF)
	...
	elseif tpr.tpn_list[name] then
		name2 = tpr.tpn_list[name]
		...
		if tpr.tpr_list[name2] then
			tpr.tpr_list[name2] = nil

		elseif tpr.tphr_list[name2] then
			tpr.tphr_list[name2] = nil

		elseif tpr.tpc_list[name2] then
			tpr.tpc_list[name2] = nil
		end
...
function tpr.tpr_accept(name)
	local no_areas_message = "Usage: /tpy allows you to accept teleport requests sent to you by other players."
	local areas_message = "Usage: /tpy allows you to accept teleport/area requests sent to you by other players."

	if not tpr.tpr_list[name] and not tpr.tphr_list[name]
	and not tpr.tpc_list[name] then
		if check_area_requests then
			minetest.chat_send_player(name, S(areas_message))
			if minetest.get_modpath("chat2") then
				chat2.send_message(minetest.get_player_by_name(name), S(areas_message), 0xFFFFFF)
	...
	if check_area_requests and tpr.tpc_list[name] then
		...
		if not tpc_target_coords[name] then
			...
			tpr.tpp_teleport_player(name2, tpc_target_coords[name2])

			chatmsg = S("@1 is teleporting to your protected area @2.", name2, minetest.pos_to_string(tpc_target_coords[name2]))
		else
			tpr.tpp_teleport_player(name2, tpc_target_coords[name])
			chatmsg = S("@1 is teleporting to your protected area @2.", name2, minetest.pos_to_string(tpc_target_coords[name]))
		end

		-- Don't allow re-denying requests.
		if tpr.tpn_list[name] or tpr.tpn_list[name2] then
			tpr.tpn_list[name] = nil
			tpr.tpn_list[name2] = nil
		end
	...
	if minetest.check_player_privs(name2, {tp_admin = true}) == false then
		...
		if tpr.enable_immediate_teleport then return end
...
function tpr.tpj(player, param)
	...
	tpr.tpc_teleport_player(player)
...
if tpr.enable_tpp_command then
	function tpr.tpp(player, param)
		-- Show the available places to the player (taken from shivajiva101's POI mod, thanks!).
		if param == "" then
			local places = {}
			if not tpr.available_places then tpr.available_places = {} end
			for key, value in pairs(tpr.available_places) do
				if minetest.get_modpath("chat2") then
		...
		elseif tpr.available_places[param] then
			local pos = {x = tpr.available_places[param].x, y = tpr.available_places[param].y, z = tpr.available_places[param].z}
			tpr.tpp_teleport_player(player, pos)
		...
		elseif not tpr.available_places[param] then
